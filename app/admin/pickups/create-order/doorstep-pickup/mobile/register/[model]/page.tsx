    ...
    const sendOtp = async () => {
      ...
      try {
        const res = await fetch("/api/otp/send", {
          ...
          body: JSON.stringify({
            identifier: phone,
            message: "starting your Mobitech pickup process", // or any custom message
          }),
        });
        const data = await res.json();
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          if (step === 4) {
                            ...
                            if (valid) {
                              console.log("Validations passed", valid);
                              // Validate final amount
                              if (!finalAmount || finalAmount.trim() === "") {
                                valid = false;
                                // toast.error("Final amount is required");
                          ...
                          onClick={async () => {
                            ...
                            if (valid && paymentMode === "upi") {
                              if (!upiId || !upiIdSaved) {
                                valid = false;
                                toast.error(
                                  "Please enter and save a valid UPI ID"
                                );
                              }
                            }
                            ...
                            if (valid && paymentMode === "bank") {
                              if (
                                ...
                                valid = false;
                                toast.error(
                                  "Please complete and save bank details"
                                );
                              }
                            }
                            ...
                            if (valid && paymentMode === "exchange") {
                              if (!exchangeModel || !exchangeImei) {
                                valid = false;
                                toast.error(
                                  "Please select exchange model and enter IMEI"
                                );
                              }
                            }
                            ...
                            if (valid && paymentMode === "cash") {
                              if (!form.watch("cashPaymentReceiptImage")) {
                                valid = false;
                                toast.error(
                                  "Cash payment receipt image is required"
                                );
                            ...
                            if (valid && paymentStatus === "paid") {
                              if (
                                ...
                                valid = false;
                                toast.error(
                                  "Please fill all payment details for paid status"
                                );
                              }
                            }
                            ...
                            if (valid && form.watch("deviceReset") === "done") {
                              if (!form.watch("deviceStartScreenImage")) {
                                valid = false;
                                toast.error(
                                  "Device start screen image is required after reset"
                                );
