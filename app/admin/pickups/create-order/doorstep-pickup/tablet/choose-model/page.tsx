...
const ChooseModel = () => {
  ...
  const getBrandData = async (brand: string | null) => {
    ...
    try {
      const res = await fetch("/api/v3/get-phone-details", {
        ...
        body: JSON.stringify({ brand: brand, device: "tablets" }),
      });
  ...
  const handleBackToBrands = () => {
    const path = pathname.split("?")[0]; // remove query string
  ...
  const handleModelSelect = (modelId: string) => {
    router.push(`/admin/pickups/create-order/doorstep-pickup/tablet/register/${modelId}`);
  };
          ...
          {loading ? (
            ...
            <div className="flex flex-col h-full">
              <button
                ...
                className="flex items-center text-blue-500 hover:text-blue-700 text-sm ml-4 mb-4"
              >
                <svg
              ...
              {filteredDevices.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-center px-4">
                  <p className="text-gray-600">
                    No Tablets found for selected brand
                  </p>
                </div>
              ) : (
